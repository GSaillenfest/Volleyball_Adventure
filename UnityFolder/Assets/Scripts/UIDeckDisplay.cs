...
public class UIDeckDisplay : MonoBehaviour
{
    ...
    public void PositionCardInHand()
    {
        ...
        if (cardCount == 1)
        {
            ...
            transform.GetChild(0).GetComponent<RectTransform>().pivot = pivotPoint;
            transform.GetChild(0).GetComponent<RectTransform>().rotation = Quaternion.Euler(0, 0, -minAngle);
        }
        else
        {
            minAngle = maxAngle / (cardCount - 1);
